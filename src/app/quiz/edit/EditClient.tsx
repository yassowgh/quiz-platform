  ...
  const save = async (publish?: boolean) => {
    ...
    if (publish) {
      questions = quiz.questions.filter(
        (qq) => qq.type === "typeanswer" || qq.type === "wordcloud" || qq.type === "openended" || qq.type === "rating" || qq.type === "scale" || (qq.options || []).some((o) => o && o.trim())
      );
      if (!questions.length) {
  ...
  if (loadError) return (
    <div className="flex flex-col items-center justify-center min-h-screen gap-3 px-6 text-center">
      ...
      <div className="flex flex-wrap gap-2 justify-center mt-2">
        ...
        <button onClick={() => router.push("/dashboard")} className="px-4 py-2 bg-kahoot-purple text-white rounded-xl font-bold">{t("Back to Dashboard")}</button>
      </div>
    </div>
  );
    ...
    <div className="max-w-3xl mx-auto p-6">
      ...
      <div className="flex flex-col gap-4 mb-8">
        ...
        <label className="flex items-start gap-3">
          <input type="checkbox" checked={!!(quiz as any).notifyOnSubmission} onChange={(ev) => setQuiz({ ...quiz, notifyOnSubmission: ev.target.checked } as any)} className="mt-1 w-5 h-5" />
          <span>
