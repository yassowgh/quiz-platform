  ...
  const next = async () => {
    ...
    if (idx + 1 >= quiz.questions.length) {
      ...
      try {
        ...
        if ((quiz as any).notifyOnSubmission === true && (quiz as any).creatorEmail) try {
          fetch("https://polished-shadow-f08c.yassow.workers.dev/", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "email", to: (quiz as any).creatorEmail, subject: "New completion: " + quiz.title, html: "Assignment completed\nQuiz: " + quiz.title + "\nBy: " + (name.trim() || "Anonymous") + "\nScore: " + finalScore + " - Correct: " + finalCorrect + "/" + quiz.questions.length + "\nQuizId: " + quizId, email: (ccEmail.trim() || (user && user.email) || "anonymous") }) }).catch(() => {});
        } catch (e) {}
        try {
