...
export function GlobalErrorListener() {
  ...
  useEffect(() => {
    ...
    function onRej(e: PromiseRejectionEvent) { const r: any = e.reason; if (!r || !(r.stack || r.message)) return; if (r.message && (r.message.indexOf("insufficient permissions") >= 0 || r.message.indexOf("Indexed Database") >= 0 || r.message.indexOf("IndexedDB") >= 0 || r.message.indexOf("Load failed") >= 0 || r.message.indexOf("NetworkError") >= 0)) return; if (isExtensionNoise(r.stack || "") || isExtensionNoise(r.message || "")) return; const detail = describeReason(r) + "\npage=" + (typeof location !== "undefined" ? location.href : ""); throttledReport("Unhandled promise rejection", detail); if (isSdkNoise(detail)) return; setToast({ msg: r.message || r.name || "Something went wrong" }); }
